...
bot.on('conversationUpdate', function (message) {
    if (message.membersAdded && message.membersAdded.length > 0) {
        if (YaNoEntres) {
            ...
            var txt = Saludo + ' \xBFEn que puedo ayudarte?';
...
bot.dialog('/', dialog);
dialog.matches('IniciarSesion', [function (session) {
...
dialog.matches('Saludo', [function (session) {
    if (!SigueSaludando) {
        session.send('Hola');
...
dialog.matches('Comprar', [async function (session, args, next) {
    ...
    var ExtensionEntidad = Producto.length,
        ExtensionPrecio = Precio.length,
        ExtensionImagen = Imagen.length;
...
dialog.matches('Pagar', [function (session) {
    funciones.GuiaPago(session);
}]);
dialog.matches('Carrito', [function (session) {
    funciones.GuiaCarrito(session);
}]);

bot.dialog('/ComprarSEntidad', [function (session) {
    console.log('====================================');
    session.send("¡Genial!");
...
bot.dialog('/verPrecio', [function (session) {
