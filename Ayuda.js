...
var Tarjetas1 = [];
var Guia = function Guia(session) {
...
var traerProductos = function traerProductos(session, ProductosElegidos) {
    ...
    for (var i = 0; i < Extension; i++) {
        CrearTarjetProductos(session, Extension - 1, "Producto" + i, ProductosElegidos[i].URLimg, i, ProductosElegidos[i].Producto, ProductosElegidos[i].Precio);
    }
    ProductosElegidos = [];
};

var CrearTarjetProductos = function CrearTarjetProductos(session, Extension, Nombre, UrlImg, ExtensionActual, NombreProducto, Precio) {
    // base64Img.img('data:image/jpeg;'+Base64Img,'', Nombre, function(err, filepath) {});

    Nombre = new builder.HeroCard(session).title(NombreProducto).subtitle("$" + Precio + ".00").text('Short Description here').images([builder.CardImage.create(session, UrlImg)]).buttons([builder.CardAction.openUrl(session, 'http://itecormovil.com/cosette/magento/index.php/shop.html', 'Comprar')]);
...
var GuiaPago = function GuiaPago(session) {

    console.log("llego a guiapago");

    session.send("Okey deja te ayudo");
    session.send("Primero debes de estar logeado");
    var Pago = new builder.HeroCard(session).title("Ahora dirigite a tu carrito para hacer el pago de tus productos").buttons([builder.CardAction.openUrl(session, 'http://itecormovil.com/cosette/magento/index.php/checkout/cart/', 'Carrito')]);
    var msjs = [];
    msjs.push(Pago);
...
var GuiaCarrito = function GuiaCarrito(session) {
    var Carrito = new builder.HeroCard(session).title("Aqui esta un acceso directo a tu carrito").buttons([builder.CardAction.openUrl(session, 'http://itecormovil.com/cosette/magento/index.php/checkout/cart/', 'Carrito')]);
    var msjs1 = [];
    msjs1.push(Carrito);
...
var login = function login(session) {
    var login = new builder.HeroCard(session).title("O haga click en el boton de abajo").buttons([builder.CardAction.openUrl(session, 'http://itecormovil.com/cosette/magento/index.php/customer/account/login/', 'Iniciar sesion')]);
    var msjs1 = [];
    msjs1.push(login);
    var msj1 = new builder.Message(session).attachmentLayout(builder.AttachmentLayout.carousel).attachments(msjs1);
    session.send(msj1);
